...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"DazaiSun_{userid}_{pack}_anim"
    elif is_video:
        return f"DazaiSun_{userid}_{pack}_vid"
    return f"DazaiSun_{userid}_{pack}"
...
def pack_nick(username, pack, is_anim, is_video):
    ...
    if is_anim:
        ...
        return f"@{username} {pack} Pack (Video)"
    else:
        return f"@{username} {pack} Pack"
...
async def pussycat(args):
    ...
    if message and message.media:
        ...
        return
    cmd = "/newvideo"
    packname = f"odi_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
